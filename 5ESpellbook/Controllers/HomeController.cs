...
namespace _5ESpellbook.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Generate(IEnumerable<string> SelectedSpells, string ofType)
        {
            ...
            if (ofType.Trim().ToLower().Contains("cards"))
            {
                return View("SpellCards", _builder.BuildSpellListModel(spells));
            }
            else if (ofType.Trim().ToLower().Contains("book"))
            {
        ...
        {
            List<Spell> spells = _repo.Spells.Where(x => x.Name.Replace(" ", "").ToLower().StartsWith(id.ToLower())).ToList();

            return View(_builder.BuildFindSpellModel(spells));
        }
        ...
        public ActionResult SpellLink(string id)
        {
            SpellLinkModel model = new SpellLinkModel();
            try
            {
                foreach (string spellId in id.Split(',').ToList())
                {
                    Spell spell = _repo.Spells.FirstOrDefault(x => x.SpellID == Convert.ToInt32(spellId));
                    if (spell != null)
                    {
